...
Chasing 10x productivity, you run several claude code agents simultaneously and push the code with blazingly fast ferocity.
No juniors left in the team, they just couldn't catch up.
This morning, walking in the park with his dog, your teamlead wrote and deployed a 100 storypoints system just by casually talking on his phone to a remote agent.
...
"It's fucking vibe code, I don't care!" - this sentiment is just a tip of the iceberg of a current industry problem:
...
The solution isn neither to abandon AI coding not to accept messy codebases. It's to **architect the separation explicitly.**

Think of it like this: Human code provides the "form" or "mold" that vibe code must fill. Just like the inpainting/outpainting feature in image generation. The human code contains your domain knowledge, business logic, and architectural decisions. The vibe code is just the implementation details that makes it work.

When AI code is clearly separated and constrained by interfaces, tests, and clear boundaries, you also can regenerate it fearlessly while keeping your valuable insights intact.
...
Share knowledge, discover new details about system.
...
There are deeper psychological and sociological nth-order benefits of this approach.
...
I believe that this approach could tip the scale for the businesses who are still indecisive about vibe coding due to reliability and security concerns.

## Looking Forward: The Tooling Evolution

I strongly believe that in near future the distinction of vibe code and human code will be admitted by industry and integrated into existing tools.

### Git-Level Integration

Git commits automatically tagged as vibed. GitHub PRs showing clear visual distinction between human and AI contributions, up to code line level.
...
For the case of monorepo, I did with the "-interface" packages that contain concentrated domain knowledge and shape-setting code (tests, interfaces).
...
Therefore, "we need to go deeper". That's an example how I defined the interface for taskmaster.generateTask, in its own package:
...
export const generateTasks: GenerateTasksF = (deps) => async (prd, current) => {
  ...
  await deps.cli.generate(path, outputPath); // don't clean up here
  // we read file system after CLI ran to return parsed tasks.json
  return await deps.readTasksJson(outputPath);
};
```

Past this point, it's already possible to tell claude code to generate the `GenerateTasksDeps` providing code that calls the CLI, saves and reads from file system.

Important details that we want to be preserved - "prd file is temporary and we want to have it in file system before calling cli", "we also want to read the result of cli call from file system" are well-preserved as strong contextual harness for llm code.

Data shape definitions is also a great candidate to use as controlling code:
...
And of course unit tests is a great candidate for putting into controlling packages, especially [property based tests](https://monadical.com/posts/property-based-testing-for-temporal-graph-storage.html).

Also, you can put there all the code that you *could* 100% vibe but better *don't* - that you're supposed to know it, if not by heart, then at least "at some point when you wrote/reviewed it".

Tell the agent to conform those interfaces, test and shape-setting functions, writing the "-implementation" package counterpart to your "-interface". An example system query: https://github.com/Monadical-SAS/taiga-taskmaster/blob/master/.llm-docs/PACKAGE_ORGANISATION_AND_CODE_SEPARATION_STANDARDS.md

To me, it worked handsomely. Agent was very strong at regenerating code anew if I didn't like something and wanted to add more context to the "control packages". And it never loses any context because of strict conformance to controlling code.

And for PR process, it's now becomes clear which code it worth more attention and which you can just glance over, which unlocks much more of vibe power without compromising quality.